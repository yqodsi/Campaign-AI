...
export default function CampaignDetailPage() {
  ...
  const formatScheduleDay = (scheduleDay: number) => {
    // scheduleDay encodes baseDay * 10000 + emailIndex
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="emails">
            Emails ({campaign.scheduledEmails.length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="emails">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                View and approve scheduled emails for this campaign
              </CardDescription>
            </CardHeader>
            <CardContent>
              {campaign.scheduledEmails.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Emails will be scheduled automatically when the campaign is
                    active
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  ...
                  {campaign.scheduledEmails.filter((e) => e.status === "READY")
                    .length > 0 && (
                    <div className="border-l-4 border-l-yellow-500 bg-yellow-50 p-4 rounded-r-md">
                      <div className="flex items-center justify-between mb-2">
                        <h3 className="font-semibold text-yellow-900">
                          ⏳ Pending Review (
                          {
                            campaign.scheduledEmails.filter(
                              (e) => e.status === "READY"
                            ).length
                          }
                          )
                        ...
                        <Button
                          size="sm"
                          onClick={async () => {
                            const readyEmails = campaign.scheduledEmails.filter(
                              (e) => e.status === "READY"
                            );
                            for (const email of readyEmails) {
                              try {
                                await fetch(`/api/emails/${email.id}/approve`, {
                                  method: "POST",
                                });
                              } catch (error) {
                                console.error(
                      ...
                      <p className="text-sm text-yellow-800">
                        These emails are ready for review. Approve them to send
                        ...
                        {campaign.scheduledEmails
                          ...
                          .map((email) => (
                            <TableRow key={email.id}>
                              ...
                              <TableCell>
                                <Badge
                                  variant={getEmailStatusColor(email.status)}
                                  className="text-xs"
                                >
                                  {email.status}
                                </Badge>
                                    ...
                                    <Dialog>
                                      ...
                                      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                                        <DialogHeader>
                                          ...
                                          <DialogDescription>
                                            Email for {email.lead.firstName} (
                                            {email.lead.email})
                                          </DialogDescription>
                                        </DialogHeader>
